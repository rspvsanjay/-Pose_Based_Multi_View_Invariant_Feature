...
mpath = '/DATA/sanjay/VT-GAN-master/CASIA_B/CASIA_B_pose_data_to_train_CNN/GEINet_Model_with_Pose_length_5for_180.h5'
model = load_model(mpath)
path1 = '/DATA/sanjay/VT-GAN-master/CASIA_B/CASIA_B180degree_Centered_Alinged/'
save_path = '/DATA/sanjay/VT-GAN-master/CASIA_B/CASIA_B180degree_Centered_Alinged_PEI_5/'
train_imgs = []
train_labels = []
test_imgs = []
test_labels = []
...
for i in range(1, numberOfSubject + 1):  #
    ...
    for j in range(0, numberOfsequences):  # numberOfsequences
        ...
        for number1 in range(0, poses):
            ...
            print(pose[number1].max(), ' :max1')
            #pose[number1-1] = ((pose[number1-1] / max1)*255)
            pose[number1] = (pose[number1] / pose[number1].max())*255
            cv2.imwrite(path2save, pose[number1])
